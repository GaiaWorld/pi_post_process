use crate::{geometry::{Geometry, vertex_buffer_layout::{EVertexBufferLayout, get_vertex_buffer_layouts}}, material::{blend::{get_blend_state, EBlend}, shader::{Shader, EPostprocessShader}, tools::{ effect_render, get_uniform_bind_group, VERTEX_MATERIX_SIZE, DIFFUSE_MATERIX_SIZE, SimpleRenderExtendsData, UniformBufferInfo, TextureScaleOffset}}, effect::blur_radial::BlurRadial, postprocess_pipeline::{PostProcessMaterialMgr, PostprocessMaterial, PostprocessPipeline}, temprory_render_target:: EPostprocessTarget };
...
pub fn blur_radial_render<'a>(
    ...
    let pipeline = postprocess_pipelines.get_material(EPostprocessShader::BlurRadial).get_pipeline(target, &primitive, depth_stencil);
    ...
    update_uniform(renderer, &queue, blur_radial, (texture_scale_offset.use_w, texture_scale_offset.use_h));
    effect_render(
        queue,
        renderpass,
        geometry,
