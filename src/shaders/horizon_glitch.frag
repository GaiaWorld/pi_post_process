...
layout(location = 1) in vec2 vGlitch;
...
void main() {
    ...
    vec4 src2 = texture(sampler2D(diffuseTex, sampler_diffuseTex), vMainUV + vec2(diff_u, 0.));
    vec4 src3 = texture(sampler2D(diffuseTex, sampler_diffuseTex), vMainUV + vec2(diff_u * 2, 0.));
