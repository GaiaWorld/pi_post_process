...
use crate::{effect::{
    ...
    vignette::Vignette, copy::CopyIntensity, radial_wave::RadialWave, color_filter::ColorFilter, filter_sobel::FilterSobel, bloom_dual::BloomDual, horizon_glitch::HorizonGlitch},
...
impl PostProcess {
    ...
    pub fn draw_front(
        ...
        dst: (u32, u32),
    ) -> Result<EPostprocessTarget, String> {
        ...
        resource.2 = temp_targets.record_from_other(src);
        let result = self._draw_front(
            device, queue, encoder, postprocess_pipelines, geometrys, resource, dst, blend, matrix, &mut temp_targets
        );
    ...
    fn _draw_front<'a>(
        ...
        postprocess_pipelines: &PostProcessPipeline,
        geometrys: &PostProcessGeometryManager,
        mut src: (u32, u32, usize, ETexutureFormat),
        dst: (u32, u32),
        blend: EBlend,
        matrix: &[f32; 16],
    ...
    fn _draw_final(
        ...
        if count > 0 {
            ...
            let temp_result = self._draw_single(device, queue, encoder, postprocess_pipelines, geometrys, src, dst, blend, matrix, flag, temp_targets);
    ...
    fn _draw_single_front(
        ...
        let result = self._draw_single(device, queue, encoder, postprocess_pipelines, geometrys, src, (width, height, dst_id, format), blend, matrix, flag, temp_targets);
    ...
    fn _draw_single(
        ...
        match flag {
            ...
            EPostprocessRenderType::CopyIntensity => {
                let src = temp_targets.get_target(src_id).unwrap();
                let dst = temp_targets.get_target(dst_id).unwrap();
                copy_intensity_render(self.copy.as_ref().unwrap(), device, queue, encoder, postprocess_pipelines, self.renders.copy_intensity.as_ref().unwrap(), geometry, src, dst, blend, matrix);
            },
            EPostprocessRenderType::FinalCopyIntensity => {
                let src = temp_targets.get_target(src_id).unwrap();
                let dst = temp_targets.get_target(dst_id).unwrap();
                copy_intensity_render(&CopyIntensity::default(), device, queue, encoder, postprocess_pipelines, self.renders.final_copy_renderer.as_ref().unwrap(), geometry, src, dst, blend, matrix);
            },
        }
        Ok(())
    }
