...
use crate::{material::{tools::load_shader}, temprory_render_target::PostprocessTexture, effect::{blur_direct::BlurDirect, radial_wave::RadialWave}};

use super::base::{TImageEffect, ImageEffectResource, KeyPostprocessPipeline};


pub struct EffectRadialWave {
...
impl TImageEffect for EffectRadialWave {
    ...
    const KEY: &'static str = "EffectRadialWave";

    fn shader(device: &RenderDevice) -> crate::material::tools::Shader {
        load_shader(
            device,
            include_str!("../shaders/radial_wave.vert"),
